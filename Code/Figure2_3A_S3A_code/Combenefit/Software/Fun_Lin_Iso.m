...
% Function to solvee extended version of linear isobole equation
  ...
  if D1==0&&D2==0
    RES = abs(E - doseresponse(k1, D1)); % setting E to CTRL value
  elseif(D1==0)
    RES = abs(E - doseresponse(k2, D2)); % setting E to U2(D2) value
   elseif(D2==0)
    RES = abs(E - doseresponse(k1, D1)); % setting E to U1(D1) value
  else
      if(E>k1(3))
        A = D1/responsedose(k1,E);
      else
        A = 2.0; % Effect greater than D1 efficacy not acceptable
      end
      if(E>k2(3))
            B = D2/responsedose(k2,E);
      else
